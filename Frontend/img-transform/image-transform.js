// File: Frontend/img-transform/image-transform.js
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // =========================================================================
const GGUF_WORKFLOW_BLUEPRINT = { "6":{"inputs":{"text":"...","clip":["209",0]},"class_type":"CLIPTextEncode"},"11":{"inputs":{"guidance":3,"conditioning":["194",0]},"class_type":"FluxGuidance"},"27":{"inputs":{"vae_name":"ae.safetensors"},"class_type":"VAELoader"},"163":{"inputs":{"seed":1000,"steps":15,"cfg":1,"sampler_name":"euler","scheduler":"simple","denoise":0.75,"model":["210",0],"positive":["11",0],"negative":["195",0],"latent_image":["188",0]},"class_type":"KSampler"},"164":{"inputs":{"samples":["163",0],"vae":["27",0]},"class_type":"VAEDecode"},"171":{"inputs":{"images":["164",0]},"class_type":"PreviewImage"},"188":{"inputs":{"pixels":["196",0],"vae":["27",0]},"class_type":"VAEEncode"},"194":{"inputs":{"conditioning":["6",0],"latent":["188",0]},"class_type":"ReferenceLatent"},"195":{"inputs":{"conditioning":["6",0]},"class_type":"ConditioningZeroOut"},"196":{"inputs":{"image":["203",0]},"class_type":"FluxKontextImageScale"},"203":{"inputs":{"image":"placeholder.png"},"class_type":"LoadImage"},"209":{"inputs":{"clip_name1":"PLACEHOLDER.gguf","clip_name2":"clip_l.safetensors","type":"flux"},"class_type":"DualCLIPLoaderGGUF"},"210":{"inputs":{"unet_name":"flux1-kontext-dev-Q4_K_M.gguf"},"class_type":"UnetLoaderGGUF"}};
const FP8_WORKFLOW_BLUEPRINT = { "6":{"inputs":{"text":"...","clip":["193",0]},"class_type":"CLIPTextEncode"},"11":{"inputs":{"guidance":3,"conditioning":["194",0]},"class_type":"FluxGuidance"},"27":{"inputs":{"vae_name":"ae.safetensors"},"class_type":"VAELoader"},"163":{"inputs":{"seed":1000,"steps":15,"cfg":1,"sampler_name":"euler","scheduler":"simple","denoise":0.75,"model":["192",0],"positive":["11",0],"negative":["195",0],"latent_image":["188",0]},"class_type":"KSampler"},"164":{"inputs":{"samples":["163",0],"vae":["27",0]},"class_type":"VAEDecode"},"171":{"inputs":{"images":["164",0]},"class_type":"PreviewImage"},"188":{"inputs":{"pixels":["196",0],"vae":["27",0]},"class_type":"VAEEncode"},"192":{"inputs":{"unet_name":"flux1-dev-kontext_fp8_scaled.safetensors","weight_dtype": "default"},"class_type":"UNETLoader"},"193":{"inputs":{"clip_name1":"t5xxl_fp16.safetensors","clip_name2":"clip_l.safetensors","type":"flux"},"class_type":"DualCLIPLoader"},"194":{"inputs":{"conditioning":["6",0],"latent":["188",0]},"class_type":"ReferenceLatent"},"195":{"inputs":{"conditioning":["6",0]},"class_type":"ConditioningZeroOut"},"196":{"inputs":{"image":["203",0]},"class_type":"FluxKontextImageScale"},"203":{"inputs":{"image":"placeholder.png"},"class_type":"LoadImage"}};
    ...
    const queuePrompt = async () => {
        if (!ui.imageUpload.files[0]) {
            ui.statusDiv.textContent = '❌ Please upload a source image first!';
            return;
        }
    ...
    // The rest of the file (updateWorkflow, listenForProgress, etc.) remains unchanged as it's already correct.
    ...
    const getUserInput = (imageName) => {
        ...
        const uInput = { prompt: ui.positivePrompt.value, seed: ui.seedInput.value ? parseInt(ui.seedInput.value, 10) : Math.floor(Math.random() * 10 ** 15), imageName: imageName, transformType: ui.transformType.value, ...sliderValues };
    ...
    const displayFinalImage = (imageInfo, generationParams) => {
        ...
        lastGeneratedResult = { url: imageUrl, filename: `Kairo_Transformed_${generationParams.transformType}_${generationParams.seed}.png` };
        ...
        ui.generationDetails.innerHTML = `<h4>Generation Details</h4><p><strong>Model:</strong> ${modelName}</p><p><strong>Seed:</strong> ${generationParams.seed}</p><p><strong>Steps:</strong> ${generationParams.steps}</p><p><strong>Guidance:</strong> ${generationParams.guidance.toFixed(1)}</p><p><strong>Denoise:</strong> ${generationParams.denoise.toFixed(2)}</p><p><strong>Transform:</strong> ${generationParams.transformType}</p><p><strong>Prompt:</strong> ${generationParams.prompt}</p>`;
    ...
    const handleTransformTypeChange = () => {
        const selectedType = ui.transformType.value;
        ui.positivePrompt.value = DEFAULT_PROMPTS[selectedType] || '';
        const typeText = { toColor: 'line art', toLineArt: 'full color image', toWatercolor: 'photo' };
        ui.uploadText.textContent = `Click to upload ${typeText[selectedType] || 'image'}`;
    };
    ...
    const setupEventListeners = () => {
        ...
        ui.transformType.addEventListener('change', handleTransformTypeChange);
