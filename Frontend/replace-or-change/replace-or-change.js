// File: Frontend/replace-or-change/replace-or-change.js
// FINAL CORRECTED VERSION: Fixes the 'missing class_type' error and ensures client-server compatibility.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // =========================================================================
    // === THE FIX: Simplified and validated workflow structure. ==============
    // The previous structure had nesting issues that caused the error.
    // Each node, including "6", now clearly has its class_type.
    // =========================================================================
Replace
const GGUF_WORKFLOW_BLUEPRINT = { "6":{"inputs":{"text":"...","clip":["211",0]},"class_type":"CLIPTextEncode"}, "11":{"inputs":{"guidance":3,"conditioning":["194",0]},"class_type":"FluxGuidance"}, "27":{"inputs":{"vae_name":"ae.safetensors"},"class_type":"VAELoader"}, "163":{"inputs":{"seed":5000,"steps":15,"cfg":1,"sampler_name":"euler","scheduler":"simple","denoise":0.75,"model":["210",0],"positive":["11",0],"negative":["195",0],"latent_image":["188",0]},"class_type":"KSampler"}, "164":{"inputs":{"samples":["163",0],"vae":["27",0]},"class_type":"VAEDecode"}, "171":{"inputs":{"images":["164",0]},"class_type":"PreviewImage"}, "188":{"inputs":{"pixels":["196",0],"vae":["27",0]},"class_type":"VAEEncode"}, "194":{"inputs":{"conditioning":["6",0],"latent":["188",0]},"class_type":"ReferenceLatent"}, "195":{"inputs":{"conditioning":["6",0]},"class_type":"ConditioningZeroOut"}, "196":{"inputs":{"image":["203",0]},"class_type":"FluxKontextImageScale"}, "203":{"inputs":{"image":"placeholder.png"},"class_type":"LoadImage"}, "210":{"inputs":{"unet_name":"flux1-kontext-dev-Q4_K_M.gguf"},"class_type":"UnetLoaderGGUF"}, "211":{"inputs":{"clip_name1":"PLACEHOLDER.gguf","clip_name2":"clip_l.safetensors","type":"flux"},"class_type":"DualCLIPLoaderGGUF"} };
const FP8_WORKFLOW_BLUEPRINT = { "6":{"inputs":{"text":"...","clip":["193",0]},"class_type":"CLIPTextEncode"}, "11":{"inputs":{"guidance":3,"conditioning":["194",0]},"class_type":"FluxGuidance"}, "27":{"inputs":{"vae_name":"ae.safetensors"},"class_type":"VAELoader"}, "163":{"inputs":{"seed":5000,"steps":15,"cfg":1,"sampler_name":"euler","scheduler":"simple","denoise":0.75,"model":["192",0],"positive":["11",0],"negative":["195",0],"latent_image":["188",0]},"class_type":"KSampler"}, "164":{"inputs":{"samples":["163",0],"vae":["27",0]},"class_type":"VAEDecode"}, "171":{"inputs":{"images":["164",0]},"class_type":"PreviewImage"}, "188":{"inputs":{"pixels":["196",0],"vae":["27",0]},"class_type":"VAEEncode"}, "192":{"inputs":{"unet_name":"flux1-dev-kontext_fp8_scaled.safetensors" , "weight_dtype": "default" },"class_type":"UNETLoader"}, "193":{"inputs":{"clip_name1":"t5xxl_fp16.safetensors","clip_name2":"clip_l.safetensors","type":"flux"},"class_type":"DualCLIPLoader"}, "194":{"inputs":{"conditioning":["6",0],"latent":["188",0]},"class_type":"ReferenceLatent"}, "195":{"inputs":{"conditioning":["6",0]},"class_type":"ConditioningZeroOut"}, "196":{"inputs":{"image":["203",0]},"class_type":"FluxKontextImageScale"}, "203":{"inputs":{"image":"placeholder.png"},"class_type":"LoadImage"} };
    
    const DEFAULT_PROMPTS = {
        replace_background: "Replace the background with a futuristic city at night, but keep the person in the foreground exactly the same.",
        replace_character: "Replace the dog with a small, red dragon. Keep the forest background and the painterly art style.",
        change_style: "Change the style of the image to a vibrant, detailed watercolor painting.",
        change_text: 'Change the text on the sign to say "Welcome to Kairo".'
    };

    const ui = { imageUpload: document.getElementById('image-upload'), uploadPrompt: document.getElementById('upload-prompt'), imagePreview: document.getElementById('image-preview'), transformationType: document.getElementById('transformation-type'), positivePrompt: document.getElementById('positive-prompt'), seedInput: document.getElementById('seed'), randomizeSeedButton: document.getElementById('randomize-seed'), generateButton: document.getElementById('generate-button'), loader: document.getElementById('loader'), outputImage: document.getElementById('output-image'), placeholderText: document.querySelector('.placeholder-text'), statusDiv: document.getElementById('status'), actionBar: document.getElementById('action-bar'), saveButton: document.getElementById('save-button'), generationDetails: document.getElementById('generation-details'), progressContainer: document.getElementById('progress-container'), progressBarFill: document.getElementById('progress-bar-fill'), progressSteps: document.getElementById('progress-steps'), progressPercent: document.getElementById('progress-percent'), queueStatusWrapper: document.getElementById('queue-status-wrapper'), queueCount: document.getElementById('queue-count'), executingNode: document.getElementById('executing-node'), stopButton: document.getElementById('stop-button'), };
    ...
    const queuePrompt = async () => {
        ...
        try {
            ...
            // Simplified blueprint selection
    ...
    // (The rest of the functions: listenForProgress, fetchFinalImage, etc. are correct and don't need changes)
    ...
    const getUserInput = (imageName) => {
        ...
        const uInput = { prompt: ui.positivePrompt.value, seed: ui.seedInput.value ? parseInt(ui.seedInput.value, 10) : Math.floor(Math.random() * 10 ** 15), imageName: imageName, transformType: ui.transformationType.value, ...sliderValues };
    ...
    const displayFinalImage = (imageInfo, generationParams) => {
        ...
        lastGeneratedResult = { url: imageUrl, filename: `Kairo_Replaced_${generationParams.transformType}_${generationParams.seed}.png`, suggestedFilename: `Kairo_Replaced_${generationParams.transformType}_${generationParams.seed}.png` };
        ...
        ui.generationDetails.innerHTML = `<h4>Generation Details</h4><p><strong>Model:</strong> ${modelName}</p><p><strong>Seed:</strong> ${generationParams.seed}</p><p><strong>Steps:</strong> ${generationParams.steps}</p><p><strong>Guidance:</strong> ${generationParams.guidance.toFixed(1)}</p><p><strong>Denoise:</strong> ${generationParams.denoise.toFixed(2)}</p><p><strong>Transform:</strong> ${generationParams.transformType.replace(/_/g, ' ')}</p><p><strong>Prompt:</strong> ${generationParams.prompt}</p>`;
    ...
    const updatePromptForSelection = () => {
        const selection = ui.transformationType.value;
        ui.positivePrompt.value = DEFAULT_PROMPTS[selection] || '';
    };
    ...
    const setupEventListeners = () => {
        ...
        ui.transformationType.addEventListener('change', updatePromptForSelection);
